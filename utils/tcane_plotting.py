...
from utils.tcane_data_funcs import get_TCANE_distribution, make_SHASH, get_PDF_CDF, calc_pctiles, get_RI_info 
######
...
def plot_RI(ax,df,edeck_all=pd.DataFrame()):
    ...
    if not edeck_all.empty:
        ...
        colors_ref = {'xkcd:tomato','xkcd:slate grey'}
        sns.pointplot(data=edeck_trim,x=edeck_trim.index,y='Prob(RI)',hue='Technique',palette=sns.color_palette(colors_ref),alpha=0.5,ax=ax)
    #
    ax.legend(fontsize=14)
    ax.grid()
    ...
    ax.set_xticks(dff.reset_index()['index'].index)
...
def make_pctile_plot(ax,df,ttype,df_out,d_in,b_deck=pd.DataFrame()):
    ...
    ax.plot(df_i['FHOUR'],df_i['P0.5'],color=colors[3],linewidth=4,label='median')
    ax.scatter(df_i['FHOUR'],df_i['P0.5'],color=colors[3],s=80,label=None)
    # obs
    # Add info for t = 0
...
# * `Ypct`: desired percentiles of TCANE model for all forecast times for `type_sel` forecast [Dataframae]
# * `RI`: RI probability estimates for all forecast times for `type_sel` forecast [Dataframe]
# * `TC_thresh`: probability estimates of reaching Cat1, Cat2, etc strength for all forecast times for `type_sel` forecast [Dataframe]

def make_all_plotting_data(df_in,df_out,xmax,type_sel,pvc=None):
    if not pvc:
    ...
    # tcane_dist_LATE = get_TCANE_distribution(df_out,'late',df_in)
    tcane_dist = get_TCANE_distribution(df_out,type_sel,df_in)
    # Now get actual data following distributions
    V0 = df_in['VMAX0'].iloc[0] # initial intensity
    N = 100000
    Yshash = make_SHASH(N,tcane_dist,type_sel,V0)
    # Yshash_erly = make_SHASH(N,tcane_dist_ERLY,'erly',V0)
    # Yshash_late = make_SHASH(N,tcane_dist_LATE,'late',V0)
    # Get PDF/CDF
    pdf_cdf = get_PDF_CDF(xmax,Yshash,type_sel)
    # pdf_cdf_erly = get_PDF_CDF(xmax,Yshash_erly,'erly')
    # pdf_cdf_late = get_PDF_CDF(xmax,Yshash_late,'late')
    # Estimate percentiles
    Ypct = calc_pctiles(Yshash,pvc)
    # Ypct_erly = calc_pctiles(Yshash_erly,pvc)
    # Ypct_late = calc_pctiles(Yshash_late,pvc)
    # Get RI/TC thresholds
    # RI_erly,TC_thresh_erly = get_RI_info(pdf_cdf_erly,df_in,'erly')
    # RI_late,TC_thresh_late = get_RI_info(pdf_cdf_late,df_in,'late')
    RI,TC_thresh = get_RI_info(pdf_cdf,df_in,type_sel)
    # 
    #Yshash_erly = Yshash_erly.mask(Yshash_erly['Y'] < 0)
    # Yshash_late = Yshash_late.mask(Yshash_late['Y'] < 0)
    Yshash = Yshash.mask(Yshash['Y'] < 0)
    return Yshash,pdf_cdf,Ypct,RI,TC_thresh
...
# <b>Input</b>: 
# * `ax`: Figure axis
# * `df`: dataframe containing the RI probabilities [Dataframe]
...
def plot_RI_bar(ax,df,type_sel,df_clim,basin,edeck_all=pd.DataFrame()):
    ...
    RI_climo = pd.read_csv('utils/RI_percentages_TCANE_training.csv')
    ...
    # 
    colors = {'xkcd:yellow orange','xkcd:cornflower','xkcd:leaf green','xkcd:tomato','xkcd:slate grey'}
    dffx = df
    ymin = 0
    ...
    for bar, alpha in zip(axbr.containers[0], alphavals):
        bar.set_alpha(alpha)
    
    #
   
    #sns.barplot(data=edeck_trim,x=edeck_trim.index,y='Prob(RI)',hue='Technique',palette=sns.color_palette('Greys_r'),ax=ax)
    #
    # y_ticks = 
    #ax.legend(fontsize=14)
    ax.grid()
    ...
    ax.set_xlabel('Forecast Hour',fontsize=22)
    xlabelsel = ['{kt} kts/{hr} hrs'.format(kt=RI_thresh[i],hr=i) for i in dff.reset_index()['index']]
    ax.set_xticks(np.arange(0,len(dff.index.unique().tolist())))
    ax.set_xticklabels(xlabelsel[0:len(dff.index.unique().tolist())])
    ax.tick_params(axis='both',labelsize=14)
    # ax.set_title('Prob. of RI for {name} ({exdate})'.format(name=df_in.iloc[0]['NAME'],exdate=ex_date),fontsize=28)
    return ax
...
def make_TCANE_dists_pdf_cdf(dfout,dfin,type_sel):
    ...
    pvc = [.01,.05,.1,.25,.5,.75,.9,.95,.99]
    # tcane_dist_ERLY = get_TCANE_distribution(dfout,'erly',dfin)
    # tcane_dist_LATE = get_TCANE_distribution(dfout,'late',dfin)
    tcane_dist = get_TCANE_distribution(dfout,type_sel,dfin)
    # Now get actual data following distributions
    ...
    return tcane_dist,Yshash,pdf_cdf
